...
/* global p5, XXH */
/* exported preload, setup, draw, mouseClicked, increaseSnakeLen, gameOver, getScore */
...
function setup() {
  ...
  let input = createInput("inf-snake-seed");
  input.parent(label);
  input.id("daLabel")
  ...
  createP("Arrow keys change snake direction. Clicking unblocks tiles.\n\nCollecting orbs increases snake speed and length!").parent("splash-container");
...
function moveSnake() {
  ...
  if (ready && millis() > next) {
    ...
    //adjust the head
    snake_head_pos.x += snake_direction == 3 ? 1 : (snake_direction == 2 ? -1 : 0)
    snake_head_pos.y += snake_direction == 0 ? -1 : (snake_direction == 1 ? 1 : 0)
    step = base_step - snake_body.length * 20
    next += step
    score += snake_len / 2
...
function draw() {
  ...
  let camera_delta = new p5.Vector(camera_target.x - camera_offset.x, camera_target.y - camera_offset.y);

  //console.log(camera_delta)
  camera_velocity = camera_delta;
  camera_velocity.mult(0.01); // cheap easing
...
function drawTile([world_x, world_y], [camera_x, camera_y]) {
  ...
  if (window.p3_drawTile) {
    window.p3_drawTile(world_x, world_y, -screen_x, screen_y, [snake_head_pos.x, snake_head_pos.y], snake_body);
  }
  pop();
}
